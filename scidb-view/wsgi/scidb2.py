...
def querySciDB(cmd):
    ...
    f.write("starting querySciDb py")
    f.write("\ndate " + str(datetime.datetime.now()))

    proc = subprocess.Popen(["/opt/scidb/12.10/bin/iquery", "-o", "csv+", "-a", "-q", cmd], stdout = subprocess.PIPE,shell=True)
...
def queryTile(name, width, height, x, y):
    ...
    if x1 > x0 and y1 > y0:
        ...
        header, rows = querySciDB("subarray(%s,%d,%d,%d,%d)" % (name, x0, y0, x1 - 1, y1 - 1))
